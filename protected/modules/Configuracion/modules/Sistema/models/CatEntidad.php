...
class CatEntidad extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('codigoEntidad, descripcionEntidad, status, fechaApertura, prefijoEfectivo, prefijoCxC, digitosFactura, rutaRecibo, rutaReciboPaquete, statusExistencias, direccion, cp, telefono, url, id_ccosto', 'required'),
			array('digitosFactura, version', 'numerical', 'integerOnly'=>true),
			...
			array('id_ccosto', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('keyEntidades, codigoEntidad, descripcionEntidad, status, fechaApertura, prefijoEfectivo, prefijoCxC, digitosFactura, rutaRecibo, rutaReciboPaquete, statusExistencias, direccion, cp, telefono, url, id_ccosto, version', 'safe', 'on'=>'search'),
		);
	}
