...
class UserRoles extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('username, role, user_role_id', 'required'),
			array('user_role_id', 'numerical', 'integerOnly'=>true),
			array('username, role', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('keyRol, username, role, user_role_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('keyRol',$this->keyRol);
